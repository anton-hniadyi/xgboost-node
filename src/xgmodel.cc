...
void XGModel::Predict(const Nan::FunctionCallbackInfo<v8::Value> &info)
{
  ...
  auto status = XGBoosterPredict(obj->handle, mat->GetHandle(), info[MASK]->Uint32Value(context).ToChecked(), info[NTREE]->Uint32Value(context).ToChecked(), &out_len, &out_result);
